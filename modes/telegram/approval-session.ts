...
import type { ToolExecutor } from '../agent/tool-execuutor.ts'; 
import type { ActionLog } from '../agent/types.ts';

import {
  composeBeforeAfter,
  formatPatch,
} from '../agent/diff-view.ts';
...
export const approvalSessions =
  new Map<number, ApprovalSession>();
...
export function approvalDiff(
  ...
  for (const s of shells) {
    parts.push( `🖥️ Shell: ${s.details.command}` ); }
  return parts.join('\n\n').trim();
}
async function promptApproval(
  ...
  await ctx.reply(
    approvalSummary(session.pending),
    {
...
export async function finishOrApprove(
  ...
  const pending = tracker.PendingMutation();
